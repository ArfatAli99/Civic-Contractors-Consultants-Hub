...
publicVar.client_and_consultant_sign_up = function(req, res){
    if(req.body.user_type == 1){
        if(req.body.facebook_id){
            CommonValidationMiddlewareHelper.client_validate('fb_signup');

        }
        else if(req.body.google_plus_id){
            CommonValidationMiddlewareHelper.client_validate('google_plus_signup');

        }
        else{
            CommonValidationMiddlewareHelper.client_validate('manualy_signup');
        }
    }
    else{
        CommonValidationMiddlewareHelper.consultant_validate('manualy_signup');
    }
        ...
        let send_email = await new Promise(function(resolve, reject){
            if(req.body.facebook_id || req.body.google_plus_id || req.body.id){
                ...
                GenericRepository.createData(validation_data).then(validation_result=>{
                    ...
                    create_user_data.link = process.env.WEBURL+'/civicContractorsandConsultantshub/html/account-verified/'+validation_hash;//change the url here
  ...
  (async()=>{
      ...
      let generate_otp = await new Promise(function(resolve, reject){
          ...
          commonFunnction.getRandomNumber(4).then((randNum) => {
            var textbody = "civicContractorsandConsultantshub : Your signup OTP is : "+randNum;
...
//         }
//         else{
//             CommonValidationMiddlewareHelper.generateRefreshAndAccessToken(data.table, user.rows[0].id).then(async (token) =>   {
//                 var data = {};
...
//                 CommonValidationMiddlewareHelper.generateRefreshAndAccessToken(data.table, by_phone_login_result.rows[0].id).then(async (token) =>   {
//                       let data = {};
//                       data.access_token = token.accessToken
//                       data.refresh_token = token.refreshToken
//                       data.role = 'user'
...
publicVar.login = async function(req, res){
    ...
    return GenericRepository.fetchData(by_phone_login_data).then(by_phone_login_result=>{
        if(by_phone_login_result.rows.length > 0){
            ...
            CommonValidationMiddlewareHelper.generateRefreshAndAccessToken(by_phone_login_data.table, by_phone_login_result.rows[0].id).then(async (token) =>   {
                  let data = {};
                  data.access_token = token.accessToken
                  data.refresh_token = token.refreshToken
                  data.role = 'user'
        ...
        try{
            ...
            if(get_user_details.rows[0].dataValues.facebook_id || get_user_details.rows[0].dataValues.google_plus_id){
                ...
                let user_details = await new Promise(function(resolve, reject){
                    ...
                    // user_details.link = 'http://'+global.constants.WEBURL+'/civicContractorsandConsultantshub/html/reset-password/'+validation_hash;
                    user_details.link = process.env.WEBURL+'/civicContractorsandConsultantshub/html/reset-password/'+validation_hash;
   ...
   console.log(global.constants.uploads.contract_documet+'5.pdf');
      ...
      mailOptions.attachments=[{
       // utf-8 string as an attachment
            ...
            //path: '/Users/rajdebnath/project@sayanti/civicContractorsandConsultantshub-node/uploads/contract_documet/5.pdf'
