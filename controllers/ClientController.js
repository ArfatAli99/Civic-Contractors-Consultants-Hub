...
clientController.inviteConsultent=(req,res)=>{
    ...
    (async()=>{
      ...
      let user_table=await GenericRepository.fetchData(user_data);
         if(user_table.rows.length>0){
          ...
          email_data.link=process.env.WEBURL+'/civicContractorsandConsultantshub/html/project-verified/'+ validationhash;
    ...
    return res.send({status:200, message:'mail sent'});
          ...
          email_data.link=process.env.WEBURL+'/civicContractorsandConsultantshub/html/signup-consultant/'+ validationhash;
