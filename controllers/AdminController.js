...
Admincontroller.verifyAdmin = (req, res, next) => {
  ...
  return GenericRepository.fetchData(data).then((user) => {
    if (user.count > 0) {
      CommonValidationMiddlewareHelper.generateRefreshAndAccessToken(data.table, user.rows[0].id).then((token) => {
        let data = {};
...
Admincontroller.inviteToConsultant = function (req, res) {
  (async () => {
    ...
    let create_consultant_data = await new Promise(function (resolve, reject) {
      ...
      // create_consultant_data.link = global.constants.WEBURL+'/civicContractorsandConsultantshub/html/consultant-hub/consultant-form/'+validation_hash;
      create_consultant_data.link = process.env.WEBURL + '/civicContractorsandConsultantshub/html/consultant-hub/consultant-form/' + validation_hash;
...
Admincontroller.inviteArticle = function (req, res) {
  (async () => {
    try {
      ...
      admin_consultant_data.link = process.env.WEBURL + '/civicContractorsandConsultantshub/html/consultant-hub/article-form/' + validation_hash;
  ...
  (async () => {
    try {
      ...
      let user_details = await new Promise(function (resolve, reject) {
        ...
        user_details.link = process.env.WEBURL + '/civicContractorsandConsultantshub/html/civicContractorsandConsultantshub-admin/dist/reset-password/' + validation_hash;
  ...
  (async () => {
    try {
      ...
      if (get_user_details.rows[0].dataValues.email != user_fetch.rows[0].dataValues.email) {
        ...
        create_user_data.link = process.env.WEBURL + '/civicContractorsandConsultantshub/html/account-verified/' + validationhash;//change the url here
